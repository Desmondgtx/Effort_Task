...
def select_slide(slide_name):
    ...
    basic_slides = {
        ...
        'intro_block': [
            u"Ahora comenzará el experimento",
            " ",
        ...
        'Instructions_Casillas': [
            ...
            u"Mientras más presiones la barra espaciadora, más se llenará la barra en la pantalla.",
            u"¡Trata de llenar la barra lo máximo que puedas!"
        ],
        'Interlude_Casillas': [
            u"¡Muy bien! AHORA INTENTA SUPERAR TU RENDIMIENTO",
            u"¡INTENTA LLENAR LA BARRA UNA VEZ MÁS!"
        ],
        'Exit_Casillas': [
...
def show_effort_bar(target_presses, max_time=5, title_text="", is_calibration=False):
    ...
    # CORRECCIÓN: Removido el timer visual (ya no se muestra el contador de segundos)
    # seconds = USEREVENT + 3
    ...
    # CORRECCIÓN: Removido el código del timer visual
    ...
    while not done:
        for event in pygame.event.get():
            ...
            # CORRECCIÓN: Removido el manejo del evento seconds (timer visual)
            ...
            # Detect spacebar press
                ...
                # CORRECCIÓN: Removido el código de redibujar el timer
...
def take_decision(buttons_number, credits_number, title_text, max_time = 5, test = False, effort_level = None, condition = None):
    ...
    # CORRECCIÓN: Usar el mismo tamaño que en show_effort_preview (mitad del tamaño original)
    # Standard image size for both effort and rest circles
    standard_circle_size = 350
    
    # CORRECCIÓN: Ajustar posición del texto para evitar solapamiento
    # Work option - credits text (elevado más arriba)
    text = font.render(f"{credits_number} créditos", True, text_color)  # Use condition color
    text_rect = text.get_rect(centerx=work_x, centery=content_y - 150)  # Más arriba para evitar solapamiento
    screen.blit(text, text_rect)
    ...
    if effort_level is not None:
        try:
            ...
            # Scale to standard size (half of original)
    ...
    # CORRECCIÓN: Ajustar posición del texto para evitar solapamiento
    # Rest option - text (elevado más arriba para igualar)
    text = font.render("1 crédito", True, text_color)  # Use condition color
    text_rect = text.get_rect(centerx=rest_x, centery=content_y - 150)  # Igualado con work option
    screen.blit(text, text_rect)
    ...
    try:
        ...
        # Scale to standard size (same as effort image - half size)
        ...
        pygame.draw.circle(screen, text_color, circle_center, circle_radius, 2)  # Condition color border
        fallback_font = pygame.font.Font(None, 24)
        text = fallback_font.render("Descanso", True, text_color)  # Condition color text
        text_rect = text.get_rect(center=circle_center)
        screen.blit(text, text_rect)
    ...
    # CORRECCIÓN: Removido el timer visual
    # seconds = USEREVENT + 3
    # pygame.time.set_timer(seconds, 1000)
    ...
    while not done:
        for event in pygame.event.get():
            ...
            # REPLACED ARROW KEYS WITH N AND M KEYS
                ...
                text_rect = text.get_rect(centerx=work_x, centery=content_y - 150)
                screen.blit(text, text_rect)
                ...
                text_rect = text.get_rect(centerx=rest_x, centery=content_y - 150)
                screen.blit(text, text_rect)
            ...
            # CORRECCIÓN: Removido el manejo del evento seconds (timer visual)
            # Pero mantenemos el timeout basado en tiempo transcurrido
...
def show_resting(title_text, max_time = 5):
    ...
    # CORRECCIÓN: Removido el timer visual pero mantenemos el timeout
    tw = pygame.time.get_ticks()

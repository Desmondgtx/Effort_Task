...
from pygame.locals import FULLSCREEN, USEREVENT, KEYUP, K_SPACE, K_RETURN, K_ESCAPE, QUIT, Color, K_c, K_UP, K_DOWN
...
def take_decision(buttons_number, credits_number, title_text, max_time = 6, test = False):
    ...
    if test:
        ...
        # text up the mouse image - MODIFIED TEXT
        font = pygame.font.Font(None, 36)
        text = font.render("Presiona la flecha hacia arriba ↑", True, (0, 0, 0))
        text_rect = text.get_rect(center=(screen.get_rect().width/10*8, row - 40))
        screen.blit(text, text_rect)

        # text down the mouse image - MODIFIED TEXT
        text = font.render("Presiona la flecha hacia abajo ↓", True, (0, 0, 0))
    ...
    # Mouse visibility remains hidden (no change needed)
    ...
    while not done:
        for event in pygame.event.get():
            ...
            # REPLACED MOUSE WHEEL WITH ARROW KEYS
    ...
    for _ in range(blocks_number):
        ...
        for combination in actual_combinations_list:
            ...
            if selection == 1:
                ...
                show_resting(f"Créditos para {combination[2]}", max_time = 10)
                earned_credits = 1
                # Initialize these variables for resting trials
                presses_done = 0
                target_reached = True
                first_press_time = None
                last_press_time = None
                ...
                earned_credits = 0
                # Initialize these variables for no decision trials
                presses_done = 0

...
def take_decision(buttons_number, credits_number, title_text, max_time = 6, test = False):
    ...
    button_positions = [3, 5]  # Left and right positions

    shuffle(button_positions)
    # Create 2 buttons horizontally arranged
    button1 = pygame.Rect((resolution[0]/6*button_positions[0] - resolution[0]/8, resolution[1]/2 - resolution[1]/8), (resolution[0]/4, resolution[1]/4))
    button2 = pygame.Rect((resolution[0]/6*button_positions[1] - resolution[0]/8, resolution[1]/2 - resolution[1]/8), (resolution[0]/4, resolution[1]/4))
    ...
    if button_positions[0] == 3:  # First button is on the left
    ...
    while not done:
        for event in pygame.event.get():
            ...
            # REPLACED ARROW KEYS WITH N AND M KEYS
                ...
                reaction_time = pygame.time.get_ticks() - tw
                key_pressed = "right"
                if button_positions[0] == 3:  # Second button is on the right
                    selected_button = 2
                else:  # First button is on the right
                    selected_button = 1
                done = True
...
def task(self_combinations, other_combinations, blocks_number, block_type, max_answer_time, 
    ...
    for _ in range(blocks_number):
        ...
        for combination in actual_combinations_list:
            ...
            # Selection
            selection, key_pressed, decision_reaction_time = take_decision(combination[0], combination[1], f"Créditos para {combination[2]}", max_time = max_decision_time, test = test)

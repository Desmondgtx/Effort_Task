...
def take_decision(buttons_number, credits_number, title_text, max_time = 5, test = False, effort_level = None, condition = None):
    ...
    text_rect = text.get_rect(centerx=work_x, centery=content_y - 130)  # Ajustado para círculos más pequeños
    screen.blit(text, text_rect)
    ...
    text_rect = text.get_rect(centerx=rest_x, centery=content_y - 130)  # Ajustado para círculos más pequeños
    screen.blit(text, text_rect)
    ...
    while not done:
        for event in pygame.event.get():
            if event.type == KEYUP and event.key == K_ESCAPE:
                ...
                # CORRECCIÓN BUG: Limpiar eventos después de la decisión
                pygame.event.clear()
                done = True
